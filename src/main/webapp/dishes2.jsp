...
function updatedish(i,dish_id) {

	
var name=document.getElementById("dish_name"+i).value;
var price=document.getElementById("dish_price"+i).value;
var quantity=document.getElementById("dish_quantity"+i).value;

window.location.href="updatedish?dish_id="+dish_id+"&dish_name="+name+"&price="+price+"&quantity="+quantity;
		...
		<div class="row" style="height: 300px; text-align: center;">
			<div class="col-md-10 col-md-push-1">
				<table class="table table-striped">
					...
					<% int i=0;%>
				<c:forEach items="${sessionScope.dlist }" var="d1">
				    <tr>
						<td>${d1.dish_id }</td>
						<td><input id="dish_name<%=i %>"+ value="${d1.dish_name}"> </td>
						<td><input id="dish_price<%=i %>"+ value="${d1.price}"></td>
						<td><input id="dish_quantity<%=i %>"+ value="${d1.quantity }"></td>
						<td><input type="button" value="修改" onclick="updatedish(<%=i %>,${d1.dish_id})"></td>
					</tr>
				<% i++; %>
				</c:forEach>
				</table>
				...
				<c:if test="${sessionScope.pageno!=null }">
					...
					<li><a href="dodishes?flag=up&sign=${1}">上一页</a></li>
					
					<li><a href="dodishes?flag=down&sign=${1}">下一页</a></li>
				</c:if>
